...
const listarPedido = async (req, resp) => {
    ...
    try {
        ...
        if (nome) {
            where['$Pedido.nome$'] = { // Usando a sintaxe correta para incluir a condição no modelo relacionado
                [Op.iLike]: `%${nome}%`
            };
        }

        // Contando sem o limit e offset para poder criar a paginação
        const countAll = await Pedido.count({
            where,
            include: [
                {
                    model: Pedido,
        ...
        const msg = 'Pedido. Erro ao tentar listar (Pedido)';
