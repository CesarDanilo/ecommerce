...
const listarPedido = async (req, resp) => {
    ...
    try {
        ...
        const countAll = await Pedido.count({ where });
        ...
        const result = await Pedido.findAll({
            ...
            order: orderOptions,
            where,
            attributes: ['id', 'usuario_id', 'status', 'valorTotal', 'createdAt', 'updatedAt'] // Incluindo apenas os atributos necessários
        });
        ...
        const msg = 'Erro ao tentar listar Pedido';
        console.error(error);
